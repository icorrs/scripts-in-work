...
def get_wbs_dic():
    wbs_loc = input('please enter wbs list route(1 for pythontem\'s wbs cal):')
    if wbs_loc == '1':
        wbs_loc = 'c:\\python tem\\wbs cal.xlsx'
    else:
        wbs_loc = wbs_loc
...
def get_boq_dic():
    boq_loc = input('please enter boq list route(1 for pythontem\'s boq.xlsx):')
    if boq_loc == '1':
        boq_loc = 'c:\\python tem\\boq.xlsx'
    else:
        boq_loc = boq_loc
...
def cost_cal():
    ...
    #获取计量单路径并初始化计算式为空
    cost_loc=input('please enter cost list dir route:')
    print('loading and innitializing cost_list.....')
    file_list = os.listdir(cost_loc)
    wbs_dic=get_wbs_dic()
    boq_dic=get_boq_dic()
    os.chdir(cost_loc)
    for file in file_list:
        file_path = os.path.join(cost_loc,file)
        cost_wb=openpyxl.load_workbook(filename=file)
        for sh in cost_wb:
            sh['a10']=''
